...
void init_monster_skills()
{
	...
	if (fp) {
		while (fgets(line, 1023,  fp)) {
			if(sscanf(line,"%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",&class,&skilla,&op1,&op2,&skillb,&op3,&op4,&skillc,&op5,&op6,&skilld,&op7,&op8,&skille,&op9,&op10,&skillf,&op11,&op12)!=19)
				continue;
