	...
	if (iscommand[2] == '!') {
		...
		if (strcmp(command,"!help") == 0 || strcmp(command,"!h") == 0) {
			...
			if (sd->account_id > 100000 && sd->account_id < 2000000){
				i = 3;
			}else if (sd->account_id > 95000 && sd->account_id < 100000){
				i = 2;
			}else{
				i = 1;
			}
		...
		/*------------------------------------------------------------------------------------
		  ...
		  ------------------------------------------------------------------------------------*/
		if ((sd->account_id > 95000 && sd->account_id < 100000) || (sd->account_id > 100000 && sd->account_id < 2000000)) {
			/*------------------------------------------------------------------------
			  Command:       !kill
			  Parameters:     
			  Purpose:        
			...
			if (strcmp(command,"!spawn") == 0 || strcmp(command,"!spwn") == 0 ||
				...
				if (y >= 500) {
					send_msg_p(fd, "Command Library", "Cannot exced of the max npc per map");
					return 0;
				}
				if(num_scanf_args != 1){
					if (atoi(temp) >= 1 && atoi(temp) <= 2000) { // id given, search via #.
