...
char char_ip_str[16];
long char_ip;
char char_xip_str[256];
long char_xip;
short char_port = 6121;
unsigned int char_port_fd;
...
int mmo_char_tostr(char *str, struct mmo_charstatus *p)
{
	...
	sprintf(str, "%ld\t%ld,%d\t%s\t%d,%d,%d\t%d,%d,%d\t%d,%d,%d,%d\t%d,%d,%d,%d,%d,%d\t%d,%d,%d"
		"\t%d,%d,%d,%d,%d\t%ld,%d,%ld,%ld\t%d,%d,%d\t%d,%d,%d,%d,%d"
		"\t%s,%d,%d\t%s,%d,%d",
		...
		p->party_id, p->party_status, p->guild_id, p->pet_id,
...
int mmo_char_fromstr(char *str, struct mmo_charstatus *p)
{
	...
	if ((set = sscanf(str, "%ld\t%ld,%d\t%[^\t]\t%d,%d,%d\t%d,%d,%d\t%d,%d,%d,%d\t%d,%d,%d,%d,%d,%d\t%d,%d,%d"
			  "\t%d,%d,%d,%d,%d\t%ld,%d,%ld,%ld\t%d,%d,%d\t%d,%d,%d,%d,%d"
			  "\t%[^,],%d,%d\t%[^,],%d,%d%n",
			  &tmp_long[0], &tmp_long[1], &tmp_int[2], p->name,
			  ...
			  &tmp_int[24], &tmp_int[25], &tmp_int[26], &tmp_long[2],
			  &tmp_int[28], &tmp_long[3], &tmp_long[4], &tmp_int[31], &tmp_int[32], &tmp_int[33],
			  &tmp_int[34], &tmp_int[35], &tmp_int[36], &tmp_int[37],
			  &tmp_int[38], p->last_point.map,
			  &tmp_int[39], &tmp_int[40], p->save_point.map, &tmp_int[41],
			  &tmp_int[42], &next)) != 46) {
		tmp_int[28] = 0;
		set = sscanf(str, "%ld\t%ld,%d\t%[^\t]\t%d,%d,%d\t%d,%d,%d\t%d,%d,%d,%d\t%d,%d,%d,%d,%d,%d\t%d,%d,%d"
			     "\t%d,%d,%d,%d,%d\t%ld,%d,%ld,%ld\t%d,%d,%d\t%d,%d,%d,%d,%d"
			     "\t%[^,],%d,%d\t%[^,],%d,%d%n",
			     &tmp_long[0], &tmp_long[1], &tmp_int[2], p->name,
			     &tmp_int[3], &tmp_int[4], &tmp_int[5], &tmp_int[6],
	...
	p->char_id = tmp_long[0];
	p->account_id = tmp_long[1];
	...
	p->karma = tmp_int[25];
	p->manner = tmp_int[26];
 	p->party_id = tmp_long[2];
	p->party_status = tmp_int[28];
	p->guild_id = tmp_long[3];
	p->pet_id = tmp_long[4];
	p->hair = tmp_int[31];
	p->hair_color = tmp_int[32];
	p->clothes_color = tmp_int[33];
	p->weapon = tmp_int[34];
	p->shield = tmp_int[35];
	p->head_top = tmp_int[36];
	p->head_mid = tmp_int[37];
	p->head_bottom = tmp_int[38];
	p->last_point.x = tmp_int[39];
	p->last_point.y = tmp_int[40];
	p->save_point.x = tmp_int[41];
	p->save_point.y = tmp_int[42];
	if (set != 46)
		return 0;
...
int mmo_storage_tostr(char *str, struct mmo_charstatus *p)
{
	...
	sprintf(str, "%ld,%d", p->account_id, p->storage_amount);
	strcat(str, "\t");
...
int mmo_pet_tostr(char *str, struct mmo_charstatus *p)
{
	...
	sprintf(str,"%ld,%d,%s\t%ld,%ld,%d,%d,%d,%d,%d,%d,%d",
...
static void mmo_char_init(void)
{
	...
	char_dat = calloc(sizeof(*char_dat) * 256, 1);
	if (!char_dat) {
		debug_output("mmo_char_init: Cannot allocate space on 'char_dat'.\n");
		abort();
	}
	char_max = 256;
	FILE *fp = fopen("save/characters/characters.txt", "rt");
	if (fp) {
		while (fgets(line, 65535, fp)) {
			if (char_num >= char_max) {
				char_max += 256;
				char_dat = realloc(char_dat, sizeof(*char_dat) * char_max);
			}
			memset(&char_dat[char_num], 0, sizeof(*char_dat));
			...
			if (ret) {
				...
				FILE *fps = fopen("save/storages/storage.txt", "rt");
				if (fps) {
					while (fgets(line, 65535, fps)) {
						set = sscanf(line, "%ld", &account_id);
						if (set != 1)
							continue;

						if (char_dat[char_num].account_id == account_id)
							mmo_storage_fromstr(line, &char_dat[char_num]);

					}
					fclose(fps);
				}
				else {
					debug_output("mmo_char_init: Fail to open 'save/storages/storage.txt'.\n");
					abort();
				}
				char_num++;
			}
		...
		debug_output("mmo_char_init: Fail to open 'save/characters/characters.txt'.\n");
		abort();
	}
}
...
static void mmo_pet_init(void)
{
	int ret;
	char line[65536];
	pet_dat = calloc(sizeof(*pet_dat) * 256, 1);
	if (!pet_dat) {
		debug_output("mmo_pet_init: Cannot allocate space on 'pet_dat'.\n");
		abort();
	}
	pet_max = 256;
	FILE *fp = fopen("save/pets/pet.txt", "rt");
	if (fp) {
		while (fgets(line, 65535, fp)) {
			if (pet_num >= pet_max) {
				pet_max += 256;
				pet_dat = realloc(pet_dat, sizeof(*pet_dat) * pet_max);
			}
			memset(&pet_dat[pet_num], 0, sizeof(*pet_dat));
		...
		debug_output("mmo_pet_init: Fail to open 'save/pets/pet.txt'.\n");
		abort();
	}
}
...
{
	int i, j, leader_flag, member_index;
	char line[1024];
	...
	FILE *fp = fopen("save/parties/parties.txt", "rt");
	if (fp) {
		while (fgets(line, 1023, fp)) {
			...
			if (sscanf(line, "%ld,%hd,%hd,%[^,],%d", &party_temp.party_id, &party_temp.exp,
						...
						if (char_dat[i].party_id == party_temp.party_id) {
							if (party_temp.member_count == MAX_PARTY_MEMBERS ||
								...
								if (char_dat[i].party_status == 2) {
									...
									if (member_index == -1 && char_dat[i].skill[0].lv >= 7)
										member_index = i;
		...
		debug_output("mmo_party_init: Fail to open 'save/parties/parties.txt'.\n");
		abort();
	}
	for (i = 0; i < char_num; i++) {
...
static void mmo_char_sync(void)
{
	...
	FILE *fp = fopen("save/characters/characters.txt", "wt");
	if (fp) {
		for (i = 0; i < char_num; i++) {
...
static void mmo_storage_sync(void)
{
	...
	FILE *fp = fopen("save/storages/storage.txt", "wt");
	if (fp) {
		for (i = 0; i < char_num; i++) {
			if (flags == char_dat[i].account_id)
				continue;

			mmo_storage_tostr(line, &char_dat[i]);
			fprintf(fp, "%s\n", line);
			flags = char_dat[i].account_id;
		}
	}
	fclose(fp);
}
...
static void mmo_pet_sync(void)
{
	...
	FILE *fp = fopen("save/pets/pet.txt", "wt");
	if (fp) {
		for (i = 0; i < pet_num; i++) {
		...
		if (fp2) {
			for(k = 0; k < MAX_PARTYS; k++) {
				if (party_dat[k].party_id >= 0 && party_dat[k].member_count > 0) {
					...
					fprintf(fp2, "%ld,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n", party_dat[k].party_id, char_ids[0],
...
static int make_new_char(unsigned int fd, unsigned char *dat)
{
	...
	if ((dat[24] + dat[25] + dat[26] + dat[27] + dat[28] + dat[29] > 5 * 6) || (dat[30] >= MAX_CHAR_SLOTS || dat[33] == 0) || (dat[33] >= 20 || dat[31] >= 9)) {
		debug_output("make_new_char: Error %d-%d %s %d,%d,%d,%d,%d,%d %d,%d.\n",
		fd, dat[30], dat, dat[24], dat[25], dat[26], dat[27], dat[28], dat[29], dat[33], dat[31]);
		return -1;
	}
	debug_output("make_new_char: %d-%d %s.\n", fd, dat[30], dat);
	...
	memset(&char_dat[i], 0, sizeof(*char_dat));
	...
	char_dat[i].max_sp = 2 * (100 + char_dat[i].int_) / 100;
	...
	for (a = 0; a < char_num; a++)
		if (char_dat[a].account_id == sd->account_id)
			memcpy(&char_dat[i].storage, &char_dat[a].storage, sizeof(char_dat[a].storage));
...
static int mmo_char_send006b(unsigned int fd, struct char_session_data *sd)
{
	const int offset = 24;
	int i, j, found_num;
...
static void update_user_login(long account_id, short state_auth)
{
	if (login_fd > 0) {
		debug_output("update_user_login: Update status %l-%d.\n", account_id, state_auth);
...
int parse_tologin(unsigned int fd)
{
	...
	while (RFIFOREST(fd) >= 2) {
		...
		case 0x2711:
			...
			if (RFIFOB(fd, 2) != 0) {
				if (RFIFOB(fd, 2) == 0)
					debug_output("parse_tologin: Admn account doesn't exist.\n");

				else if (RFIFOB(fd, 2) == 1)
					debug_output("parse_tologin: Wrong password for admn account.\n");

				else if (RFIFOB(fd, 2) == 2 || RFIFOB(fd, 2) == 3)
					debug_output("parse_tologin: Character Server is too old.\n");

				else if (RFIFOB(fd, 2) == 4)
					debug_output("parse_tologin: Conflicts with admn account.\n");
			...
			for (i = 5; i < FD_SETSIZE; i++) {
				if (session[i] && (sd = session[i]->session_data)) {
					if (sd->account_id == account_id) {
						...
						if (strlen(sd->email) < 3 || strlen(sd->email) > 40 || strchr(sd->email, '@') == NULL || sd->email[0] == '@' || sd->email[strlen(sd->email)-1] == '@')
							strcpy(sd->email, "NULL");

						else
							strtolower(sd->email);

						update_user_login(sd->account_id, 0);
	...
	while (RFIFOREST(fd) >= 2) {
		...
		case 0x2afc:
			...
			for (i = 0; i < AUTH_FIFO_SIZE; i++) {
				if ((unsigned)auth_fifo[i].account_id == RFIFOL(fd, 2)
				&& (unsigned)auth_fifo[i].char_id == RFIFOL(fd, 6)
				&& (unsigned)auth_fifo[i].login_id1 == RFIFOL(fd, 10)
				&& !auth_fifo[i].delflag) {
			...
			if (i == AUTH_FIFO_SIZE) {
				...
				WFIFOW(fd, 0) = 0x2afd;
				WFIFOW(fd, 2) = 12 + sizeof(*char_dat);
				WFIFOL(fd, 4) = RFIFOL(fd, 2);
				WFIFOL(fd, 8) = RFIFOL(fd, 6);
				memcpy(WFIFOP(fd, 12), &char_dat[auth_fifo[i].char_pos], sizeof(*char_dat));
				WFIFOSET(fd, WFIFOW(fd, 2));
#ifdef _MMS_
		...
		case 0x2cbf:
			...
			update_user_login(RFIFOL(fd, 2), RFIFOB(fd, 6));
			RFIFOSKIP(fd, 7);
			break;
		...
		case 0x2b01:
			...
			for (i = 0; i < char_num; i++) {
				if ((unsigned)char_dat[i].account_id == RFIFOL(fd, 4)
				&& (unsigned)char_dat[i].char_id == RFIFOL(fd, 8)) {
					memcpy(&char_dat[i], RFIFOP(fd, 12), sizeof(*char_dat));
					mmo_char_sync();
					for (j = 0; j < char_num; j++) {
						if (char_dat[i].account_id == char_dat[j].account_id && char_dat[i].char_num != char_dat[j].char_num) {
							memcpy(&char_dat[j].storage, &char_dat[i].storage, sizeof(char_dat[i].storage));
							char_dat[j].storage_amount = char_dat[i].storage_amount;
						}
					}
					mmo_storage_sync();
					break;
		...
		case 0x2c02:
			...
			for (i = 0; i < char_num; i++)
				if (char_dat[i].lastseen_map_fd || char_dat[i].lastseen_map_fd == fd || strcmp(char_dat[i].name, RFIFOP(fd, 4)) != 0)
					break;
		...
		case 0x2c06:
			...
			for (j = 0; j < MAX_SERVERS; j++) {
				if (server_fd[j] < 1 || server_fd[j] == fd)
					continue;
		...
		case 0x3a44:
			...
			for (i = 0; i < pet_num; i++) {
				if ((unsigned)pet_dat[i].pet.pet_id == RFIFOL(fd, 2)
				&& (unsigned)pet_dat[i].pet.account_id == RFIFOL(fd, 32)
				&& (unsigned)pet_dat[i].pet.char_id == RFIFOL(fd, 36))
					break;
			}
			if (i != pet_num) {
		...
		case 0x3b46:
			...
			for (i = 0; i < char_num; i++) {
				if ((unsigned)char_dat[i].pet_id == RFIFOL(fd, 2)) {
					char_dat[i].pet_id = 0;
					mmo_char_sync();
					break;
				}
			...
			for (j = 0; j < pet_num; j++) {
				if (pet_dat[j].pet.pet_id == char_dat[i].pet_id) {
					memcpy(&pet_dat[j], &char_dat[pet_num - 1], sizeof(*pet_dat));
					break;
				}
			}
			pet_num--;
...
static int search_mapserver(char *map)
{
	...
	debug_output("search_mapserver: Initial map not found on Zone Server.\n");
	return -1;
}
...
{
	int i, j, a, ch, ret;
	struct char_session_data *sd, *osd = NULL;
	...
	while (RFIFOREST(fd) >= 2) {
		...
		else if (sd->state == CHAR_STATE_WAITAUTH) {
			switch(RFIFOW(fd,0)) {
			case 0x66:
			case 0x67:
			case 0x68:
		...
		case 0x65:
			...
			for (i = 5; i < FD_SETSIZE; i++) {
				...
				if ((unsigned)i != fd && session[i] && (sd2 = session[i]->session_data) && (unsigned)sd2->account_id == RFIFOL(fd, 2)) {
					WFIFOW(fd, 0) = 0x81;
					WFIFOB(fd, 2) = 2;
					WFIFOSET(fd, 3);
		...
		case 0x68:
			...
			if (strlen(email) < 3 || strlen(email) > 40 || strchr(email, '@') == NULL || email[0] == '@' || email[strlen(email)-1] == '@')
				strcpy(email, "NULL");

			else
				strtolower(email);

			if (strncmp(email, sd->email, 40) != 0) {
				WFIFOW(fd, 0) = 0x70;
				WFIFOB(fd, 2) = 0;
				WFIFOSET(fd, 3);
			}
				...
				for (i = 0; i < MAX_CHAR_SLOTS; i++) {
					if (sd->found_char[i] >= 0 && (unsigned)char_dat[sd->found_char[i]].char_id == RFIFOL(fd, 2)) {
						debug_output("parse_char: Delete char ID %d, name %s.\n", RFIFOL(fd, 2), char_dat[sd->found_char[i]].name);
						...
						if (sd->found_char[i] != (char_num - 1)) {
							memcpy(&char_dat[sd->found_char[i]], &char_dat[char_num - 1], sizeof(*char_dat));
						...
						if (pet_id > 0) {
							for (a = 0; a < pet_num; a++) {
								if (pet_dat[a].pet.pet_id == pet_id) {
									memcpy(&pet_dat[a], &char_dat[pet_num - 1], sizeof(*pet_dat));
									break;
								}
							}
							pet_num--;
...
void check_connect_login_server(int tid, unsigned int tick, int id, int data)
{
	if (!session[login_fd]) {
...
void send_users_tologin(int tid, unsigned int tick, int id, int data)
{
	unsigned int i, fd;
	long users = 0;
...
int do_init(void)
{
	...
	if (!(fp = fopen("cfg/character_cfg.ini", "rt"))) {
		debug_output("do_init: Cannot open file 'cfg/character_cfg.ini'.\n");
		return 1;
	}
	else {
		while (fgets(line, 1023, fp)) {
			...
			if (strcmp(option1, "ServerUsername") == 0) {
				...
				login_port = (short)strtol(option2, (char**)NULL, 10);
				...
				char map[32];
				short x, y;
				if (sscanf(option2,"%[^,],%hd,%hd", map, &x, &y) < 3 )
					continue;
				...
				j[8] = 1;
				start_zeny = (long)strtol(option2, (char**)NULL, 10);
				if(start_zeny < 0)
					start_zeny = 0;
	...
	|| j[6] != 1 || j[7] != 1 || j[8] != 1) {
		debug_output("do_init: Missing configuration components.\n");
		return 1;
	}
